...
* How to run: Simply run "rails server" inside the app directory. Then browse to localhost:3000

* Demo URL: http://ec2-54-67-5-56.us-west-1.compute.amazonaws.com/

* Technologies: Rails webapp running on an Amazon EC2 instance with Mysql DB tier running in Amazaon RDS. Image annotation is done using annotorious.github.io. UI is done via Twitter's bootstrap. User authenication is done using Devise gem and achievement system is done using the merit gem.
